...
public class AddChargeToLeaseCommand {
	...
	public static class Request {
		/**
		 * Its Post a string "post_string" att the URL
		 * @param post_string
...
//				httpURLConnection.setRequestProperty("Data", "Value");
	 ...
	 * 
	 * @param httpURLConnection
	 * @return a Map<String,Sring> of the LeaseId, ID & Charge Charge 
	 */
	public static Map<String, String> ReadRequest(HttpURLConnection httpURLConnection) {
		...
		String str = response
				.subSequence(response.indexOf("LeaseID"), response.indexOf(",", response.indexOf("LeaseID")))
				.toString();
		String str3 = str.substring(str.indexOf('"', str.indexOf('"') + 1) + 1, str.length() - 1);
		String str2 = response
				.subSequence(response.indexOf("Charge"), response.indexOf("}", response.indexOf("Charge"))).toString();
	...
	public static class Response {
		 ...
		 * @param con
		 * @param str 
		 * @return a string with the Connetion code of Con
		 * @throws IOException
		 */
...
/**
 * Returns a Charge build from the string
 * @param string
 * @return
 */
	public static Charge createChargeFromString(String string) {
		...
		String date = string.substring(string.indexOf("chargeDate") + 11,
				string.indexOf(',', string.indexOf("chargeDate")));
		String year = date.substring(0, 4);
		String month = date.substring(5, 7);
		String day = date.substring(8, 10);
		String description = string.substring(string.indexOf("description") + 12,
				string.indexOf(',', string.indexOf("description") + 11));
		...
		return Charge.builder()
				...
				.amount(Money.of(CurrencyUnit.of(Current), Double.valueOf(amount))).count(Integer.valueOf(count))
